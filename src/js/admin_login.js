...
function getLoginInfo() {
    ...
    xhr.onload = function () {
        ...
        if (admin.ADMIN_ID) {
            //如果有登入
            // document.getElementById("MEM_NICKNAME").innerText = admin.MEM_NICKNAME;
            document.getElementById("spanLogin").innerText = "登出";
        }
    };
    xhr.open("get", "./phps/admin_GetLoginInfo.php", true);
    xhr.send(null);
}
...
function sendForm() {
    ...
    xhr.onload = function () {
        if (xhr.status == 200) {
            ...
            if (admin.ADMIN_ID) {
                ...
                // $id("memName").innerText = admin.memName;
                //將登入表單上的資料清空，並隱藏起來
    ...
    xhr.open("post", "./phps/admin_login.php", true);
    xhr.setRequestHeader("content-type", "application/x-www-form-urlencoded");
    let data_info = `ADMIN_ID=${$id("ADMIN_ID").value}&ADMIN_PW=${$id("ADMIN_PW").value}`;
    xhr.send(data_info);
}
